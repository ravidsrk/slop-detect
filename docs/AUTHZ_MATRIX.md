...
it. Every row is proven by the cited test file — no eyeball claims.
...
| /api/aeo | POST | none¹, gated AS scan | same as scan | authz-matrix.test.js |
| /api/fix-prompt {result} | POST | none, cheap limit | 429 past 20/min | authz-matrix.test.js |
| /api/fix-prompt {url} | POST | none, gated AS scan | same as scan | authz-matrix.test.js |
...
| /api/patterns, /api/stats, /api/sites | GET | none (public) | n/a | api-stats.test.js, sites.test.js |
...
|---|---|---|---|
| RESULTS | KV binding | all persistence | route 503s, except /api/stats → zeros, sweep → 503 |
...
| RESEND_API_KEY + ALERT_FROM | secret + var | all email | sends no-op (no_provider); flows still 200 |
