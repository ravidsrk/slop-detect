...
test(
  ...
  async () => {
    ...
    assert.equal(
      triggered(r, 'purple_accent'),
      true,
      'violet filled CTA should trip purple_accent'
    );
  }
);
...
test(
  ...
  async () => {
    ...
    assert.equal(
      triggered(r, 'hero_eyebrow_pill'),
      false,
      '"Sign up" pill button is a CTA, not a "Now in beta" eyebrow'
    );
    assert.equal(
      r.tier,
      'Clean',
      `a plain page with a CTA pill should stay Clean, got ${r.tier} (${r.score})`
    );
  }
);
...
test(
  ...
  async () => {
    ...
    assert.equal(
      triggered(r, 'hero_eyebrow_pill'),
      true,
      '"Now in beta" keyword pill must still fire'
    );
