...
function chartSvg(history) {
  ...
  return `<svg class="chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="slop score over time" preserveAspectRatio="none">
    ...
    <text x="${padX}" y="${(Number(cleanY) - 4).toFixed(1)}" fill="var(--dim)" font-size="9" font-family="var(--mono)">clean ≤10</text>
...
function axisChip(label, score, tier, suffix = '/100') {
  ...
  return `<div class="axis">
    <div class="axis-k">${escapeHtml(label)}</div>
    <div class="axis-v" style="color:${c.fg}">${score}<small>${suffix}</small></div>
    <div class="axis-t" style="color:${c.fg}">${escapeHtml(tier || '')}</div>
  </div>`;
}
...
export async function onRequestGet({ params, request, env }) {
  ...
  // ── empty state ────────────────────────────────────────────────────────────
  if (!latest) {
    const empty = `<!doctype html><html lang="en"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(domain)} · slop score · slop-detect</title>
<meta name="description" content="No slop scan recorded for ${escapeHtml(domain)} yet. Scan it to score how AI-generated its landing page looks.">
<link rel="canonical" href="${ORIGIN}/score/${escapeHtml(domain)}">
<link rel="icon" type="image/svg+xml" href="/favicon.svg">
${BRAND_FONTS_HEAD}<style>${BRAND_CSS}
  ...
  <div class="eyebrow"><span class="reg">§score</span><span class="reg-label">domain record</span></div>
  ...
  if (latest.axes && latest.axes.copy) {
    ...
    axes.push(axisChip('copy', cp.score, cp.tier));
  }
  if (latest.system) {
    axes.push(axisChip('system', latest.system.score, latest.system.tier));
  }
  ...
  const claimBlock = claimed
    ? `<div class="claim claimed">
         <div class="claim-h">Listed in the <a href="${origin}/directory">directory</a></div>
         <p>This domain is claimed and monitored. It carries a dofollow backlink:
            <a href="https://${escapeHtml(domain)}" rel="dofollow">${escapeHtml(domain)}</a>.</p>
       </div>`
    : `<div class="claim">
         <div class="claim-h">Claim this page</div>
         <p>Own ${escapeHtml(domain)}? Verify by email to list it in the directory (a real
            dofollow backlink) and get an alert if it ever drifts back into slop.</p>
         <form id="claimForm" class="claimform" novalidate>
           <input id="claimEmail" type="email" inputmode="email" placeholder="you@${escapeHtml(domain)}" required>
...
<div class="wrap">
  <div class="brand"><a href="/" style="color:inherit">slop&#8209;detect</a> <span class="slash">/ score</span></div>
  <div class="eyebrow"><span class="reg">§score</span><span class="reg-label">domain record</span></div>
  <h1>${claimed ? `<a href="https://${escapeHtml(domain)}" rel="dofollow">${escapeHtml(domain)}</a>` : escapeHtml(domain)}</h1>
  <div class="meta">last scan ${escapeHtml((latest.createdAt || '').replace('T', ' ').slice(0, 16))} UTC · defs ${escapeHtml(latest.definitionsVersion || 'n/a')}${pub?.monitoring ? ' · monitored' : ''}</div>

  <section class="hero">
    <div class="grade">${escapeHtml(latest.grade)}</div>
    <div class="hmeta">
      <div class="score">${latest.score}<small>/100</small></div>
      <div class="tier">${escapeHtml(latest.tier)}</div>
      <div class="flagged">${latest.patternsFlagged}/${latest.patternsTotal} patterns triggered · lower is better</div>
    </div>
  </section>
  ${latest.verdict ? `<div class="verdict">${escapeHtml(latest.verdict)}</div>` : ''}
  ${peerLine}

  <div class="axes">${axes.join('')}</div>

  ${chart ? `<h2>Score over time</h2><div class="chartwrap">${chart}</div>` : ''}

  <div class="actions">
    <a class="btn" href="${origin}/?url=${encodeURIComponent(domain)}">Re-scan</a>
    <a class="btn ghost" href="${origin}/r/${escapeHtml(latest.id)}">This scan</a>
    <a class="btn ghost" href="${origin}/report/${escapeHtml(domain)}">Printable report</a>
    <button class="btn ghost" id="shareX">Share</button>
  </div>

  ${tells ? `<h2>Triggered patterns</h2><ul class="tells">${tells}</ul>` : ''}
  ...
  <div class="embed">
    ...
    <div class="embed-row">
      <img src="${origin}/badge/${escapeHtml(domain)}.svg" alt="slop badge for ${escapeHtml(domain)}" height="20">
      <span style="font-size:13px;color:var(--muted)">live · re-scans periodically</span>
    </div>
    <pre id="badgeMd">[![slop](${origin}/badge/${escapeHtml(domain)}.svg)](${pageUrl})</pre>
  </div>

  <footer>Scores are deterministic, named checks: a fingerprint, not a verdict.
    Reproduce: npx slop-detect ${escapeHtml(domain)} · <a href="${origin}/">slop-detect.com</a></footer>
</div>
...
<script>
  ...
  if(sx)sx.addEventListener('click',()=>{
    const text=${JSON.stringify(`${domain} scored ${latest.grade} (${latest.score}/100) on the AI-design-slop detector. ${latest.verdict || ''}`)};
